defmodule FastestMCP.StreamableHTTPLifecycleTest do
  ...
  test "an idle-expired Session closes its GET stream and releases stream ownership" do
    ...
    assert_eventually(fn ->
      match?(
        {:ok, %{owner: owner, session_pid: ^session_pid}} when owner == stream_task.pid,
        TTLStore.get(runtime.session_stream_store, session_id)
      )
    end)

    assert_receive {:DOWN, ^session_monitor, :process, ^session_pid, :normal}, 1_000
    assert %Plug.Conn{state: :chunked} = Task.await(stream_task, 1_000)

    assert_eventually(fn ->
      TTLStore.get(runtime.session_stream_store, session_id) == {:error, :not_found}
    end)
  end

  test "a POST disconnect detaches response delivery without cancelling the operation" do
    parent = self()
    ...
    server =
      ...
      |> FastestMCP.add_tool("detached", fn _arguments, context ->
        ...
        :ok = Context.send_notification(context, "notifications/detached-test")
    ...
    refute_receive {:stream_dispatch_result, _stream_ref, _result}, 200
    refute_receive {:DOWN, _monitor_ref, :process, _pid, _reason}, 0
  end
