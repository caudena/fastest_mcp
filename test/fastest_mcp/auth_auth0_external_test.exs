defmodule FastestMCP.AuthAuth0ExternalTest do
  ...
  test "auth0 provider builds authorization redirect with real credentials" do
    client_id = System.get_env("FASTMCP_TEST_AUTH_AUTH0_CLIENT_ID")
    client_secret = System.get_env("FASTMCP_TEST_AUTH_AUTH0_CLIENT_SECRET")
    config_url = System.get_env("FASTMCP_TEST_AUTH_AUTH0_CONFIG_URL")
    audience = System.get_env("FASTMCP_TEST_AUTH_AUTH0_AUDIENCE")

    if blank?(client_id) or blank?(client_secret) or blank?(config_url) do
      assert true
    else
      server_name = "auth0-external-" <> Integer.to_string(System.unique_integer([:positive]))
      ...
      approve_conn =
        authorize_and_approve(server_name, client, "auth0-external-state", s256("auth0-external"),
      ...
      if audience && String.trim(audience) != "" do
        assert upstream_query["audience"] == audience
      end
    end
  end
  ...
  defp register_client(server_name) do
    conn =
      conn(
        ...
        "/register",
        Jason.encode!(%{
          client_name: "External Auth0 Client",
