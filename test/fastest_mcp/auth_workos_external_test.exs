defmodule FastestMCP.AuthWorkOSExternalTest do
  ...
  test "workos provider builds authorization redirect with real credentials" do
    client_id = System.get_env("FASTMCP_TEST_AUTH_WORKOS_CLIENT_ID")
    client_secret = System.get_env("FASTMCP_TEST_AUTH_WORKOS_CLIENT_SECRET")
    authkit_domain = System.get_env("FASTMCP_TEST_AUTH_WORKOS_DOMAIN")

    if blank?(client_id) or blank?(client_secret) or blank?(authkit_domain) do
      assert true
    else
      server_name = "workos-external-" <> Integer.to_string(System.unique_integer([:positive]))
      ...
      approve_conn =
        authorize_and_approve(
          server_name,
          client,
          "workos-external-state",
          s256("workos-external"),
  ...
  defp register_client(server_name) do
    conn =
      conn(
        ...
        "/register",
        Jason.encode!(%{
          client_name: "External WorkOS Client",
